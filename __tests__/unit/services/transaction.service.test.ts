...
import type { RecurrenceRule } from '../../../src/core/entities/transaction.entity'

describe('TransactionService', () => {
  let service: TransactionService
  let budgetService: BudgetService
  let mockTxRepo: ReturnType<typeof createMockTransactionRepository>
  let mockBudgetRepo: ReturnType<typeof createMockBudgetRepository>
  ...
  it('should fetch paginated transactions', async () => {
    ...
    mockTxRepo.findMany.mockResolvedValue(fakeData)
    const result = await service.findMany({})
    expect(result).toEqual(fakeData)
  })

  it('should find by id or throw NotFoundError', async () => {
    mockTxRepo.findById.mockResolvedValueOnce({ id: '1' } as any)
  ...
  it('create() should call checkAndNotify if expense', async () => {
    mockTxRepo.create.mockResolvedValue({ type: 'expense', date: new Date('2024-05-15T00:00:00Z'), categoryId: 'cat1' } as any)
  ...
  it('create() should NOT call checkAndNotify if income', async () => {
    mockTxRepo.create.mockResolvedValue({ type: 'income', date: new Date(), categoryId: 'cat1' } as any)
  ...
  it('update() should call checkAndNotify if expense', async () => {
    mockTxRepo.findById.mockResolvedValue({ id: '1' } as any)
    mockTxRepo.update.mockResolvedValue({ id: '1', type: 'expense', date: new Date('2024-10-10T00:00:00Z'), categoryId: 'cat2' } as any)
